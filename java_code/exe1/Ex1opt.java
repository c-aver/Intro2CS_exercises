...
public class Ex1opt {
	static public void main(String[] args) {
		...
		long r = max % min; // take the remainder of the division of the two numbers, when it is 0 min is
							// the GCD
		...
		if (gpcd != 1) {
			long i = 2; // the first number for GCD checking, we will never actually try to divide by 1
			while (!(gpcd % i != 0 && gpcd == i++))  // as long as gpcd has i as a divisor, also make sure gpcd didn't reach
				if (gpcd % i == 0)									// i
				gpcd /= i; // divide it by i, by the end of this loop either gpcd does not have i as a
							// divisor or is the answer
		}
		...
		System.out.println("The runtime took: " + runtimeInMicroSeconds + " micro seconds."); // print the runtime, you
																								// asked us to make it
																								// work in the same way
																								// as your
																								// implementation which
																								// is why I am making it
																								// exactly the same, I
																								// know it messes up the
